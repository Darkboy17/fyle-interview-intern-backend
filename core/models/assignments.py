...
class Assignment(db.Model):
    ...
    def upsert(cls, assignment_new: 'Assignment'):
        if assignment_new.content is None:
            assertions.assert_valid(False, 'Content cannot be null')
    ...
    def mark_grade(cls, _id, grade, auth_principal: AuthPrincipal):
        ...
        if assignment.state != AssignmentStateEnum.SUBMITTED:
            assertions.base_assert(400, 'Only submitted assignments can be graded')
    ...
    ## Added this new method to allow re-grading by Principal
    @classmethod
    def mark_regrade(cls, _id, grade, auth_principal: AuthPrincipal):
        assignment = Assignment.get_by_id(_id)
        assertions.assert_found(assignment, 'No assignment with this id was found')
        assertions.assert_valid(grade is not None, 'assignment with empty grade cannot be graded')
    ...
    @classmethod
    def get_submitted_graded_assignments_from_principal(cls):
